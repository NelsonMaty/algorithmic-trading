...
def hasPriceUpMomentum(dataframe):
    ...
    return (
        (current_value['Fast_SMA'] > previous_value['Fast_SMA']) and
        (current_value['Medium_SMA'] > previous_value['Medium_SMA']) and
        (current_value['Slow_SMA'] > previous_value['Slow_SMA']) and
        (current_value['ADXR'] > ADXR_THRESHOLD)
    )
...
def get_next_market_action(dataframe, current_position):
    ...
    if (current_position != 0):
        ...
        elif (is_market_downtrending(current_market_data) and hasPriceDownMomentum(dataframe)):
